...
export const obstacleExtents = Helpers.useRef('recursiveExtents', {} as Record<MapId, Record<ObstacleType, DiscreteExtent[]>>); 
...
export function updateObstacleMatrixWithLayer(layer: any, map: any, mapId: string, obstacleValue: ObstacleType, canOverride: boolean, algo : ObstacleAlgo, cutGeometry?: number, logTime: boolean = false){
	...
	if(logTime){
		...
		/*
		wlog(mapId);
		if(!obstacleExtents.current[mapId]){
			obstacleExtents.current[mapId] = {"0":[],"1":[],"10":[],"100":[],"20":[],"30":[]};
		}
		obstacleExtents.current[mapId][obstacleValue]= [];
		*/
		//keys.forEach((k : ObstacleType) => obstacleExtents.current[mapId][k]= [])
	...
	obstacleExtents.current[mapId][ol.obstacleValue] = [];

	switch(ol.algo){
		case 'Recursive':
			recursiveMatrixFill(worldGrid.grid, cutLayers, ol, offsetPoint, cellSize, logTime, mapId);
			break;
		default: // do nothing
	}
...
function recursiveMatrixFill(grid: number[][], subLayers : SubLayer[], oLayer: ObstacleLayer, offset: Point, cellSize: number, debug: boolean, mapId: string): void{

	subLayers.forEach(sub => {
		internalRecursiveFill(grid, sub.extent, sub.vectorSource, oLayer, offset, cellSize, debug, mapId);
	})

}

function internalRecursiveFill(grid: number[][], extent: DiscreteExtent, vecSrc: any, oLayer: ObstacleLayer, offset: Point, cellSize: number, debug: boolean, mapId: MapId): void {
	...
	if(hasIntersections){
		if(canFillAll)
		{
			...
			if(debug){
				obstacleExtents.current[mapId][oLayer.obstacleValue].push(extent);
			}
			...
			for(let j = 0; j < partitions.length; j++){
				internalRecursiveFill(grid, partitions[j], vecSrc, oLayer, offset, cellSize, debug, mapId);
			}
		}
